import { ProjectVestingType } from '@/constants';
...
export class Timeline {
  @ApiProperty({ required: true, default: dayjs.utc().add(1, 'day').unix() })
  registration_start_at: number;

  @ApiProperty({ required: true, default: dayjs.utc().add(2, 'day').unix() })
  ...
  @ApiProperty({ required: true, default: dayjs.utc().add(3, 'day').unix() })
  snapshot_start_at: number;

  @ApiProperty({ required: true, default: dayjs.utc().add(4, 'day').unix() })
  snapshot_end_at: number;
  ...
  @ApiProperty({ required: true, default: dayjs.utc().add(5, 'day').unix() })
  investment_start_at: number;

  @ApiProperty({ required: true, default: dayjs.utc().add(6, 'day').unix() })
  investment_end_at: number;
  ...
  @ApiProperty({ required: true, default: dayjs.utc().add(7, 'day').unix() })
...
class TokenInfo {
  ...
  token_distribute_time: number; // field
...
export class CreateProjectResponse extends withBaseResponse(Project) {}

export class GetProjectsResponse extends withBaseResponse(Project, {
  isArray: true,
}) {}

export class UpdateProjectDto extends OmitType(PartialType(Project), [
  '_id',
  'timeline',
  'token_info',
] as const) {
...
export class UpdateProjectResponse extends withBaseResponse(Project) {}
